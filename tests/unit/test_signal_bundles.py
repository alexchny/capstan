...
def _ob(ts: int, bid: float, ask: float, bqty: float = 1.0, aqty: float = 1.0) -> OrderBook:
	return OrderBook(
		ts=ts,
		venue="v",
		symbol="S",
...
def test_make_llca_features_basic() -> None:
	...
	assert set(f.keys()) == {"z", "depth_ratio", "imbalance", "update_rate", "health_min"}
...
def test_make_hfh_features_basic() -> None:
	fund = [
		Funding(ts=1000, venue="v", symbol="S", next_ts=2000, est_rate=0.0001, term_structure={}),
		Funding(ts=2000, venue="v", symbol="S", next_ts=3000, est_rate=0.0002, term_structure={}),
	]
